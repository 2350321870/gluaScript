...
if(answer=="ask")then
	...
	--[[if(day=="2012-5-17" and GetCurrentHour()>=4)then
		AddTopSayMenuItem("@5嗜血的帝王1","Ac13")
		AddTopSayMenuItem("@5嗜血的帝王2","Ac14")
		AddTopSayMenuItem("@5王的赏赐","Ac15")
		AddTopSayMenuItem("@5幸运战火","Ac16")
	elseif(day=="2012-5-18" or day=="2012-5-19" or day=="2012-5-20")then
		AddTopSayMenuItem("@5嗜血的帝王1","Ac13")
		AddTopSayMenuItem("@5嗜血的帝王2","Ac14")
		AddTopSayMenuItem("@5王的赏赐","Ac15")
		AddTopSayMenuItem("@5幸运战火","Ac16")
	elseif(day=="2012-5-20")then
		AddTopSayMenuItem("@5王的赏赐","Ac15")
	end]]
	if(day=="2012-5-21" or day=="2012-5-22" or day=="2012-5-23" or day=="2012-5-24")then
		
		AddTopSayMenuItem("@5天道的机缘","Ac17")
		AddTopSayMenuItem("@5天道的恩赐","Ac18")
		AddTopSayMenuItem("@5天道的寻求","Ac19")
...
elseif(answer=="Ac17")then
	local day=GetCurrentDay()
	if(day=="2012-5-22")then
		LuaSay("此活动暂时关闭")
	elseif(day=="2012-5-23")then
		if(GetCurrentHour()<5)then	
			LuaSay("此活动暂时关闭")
		else
			LuaSay("活动期间内每日可在建康城城中心NPC天道领取一次奖励机不可失失不再来哦！每日仅有一次哦。奖品有：2颗暴击丸、1个恩赐锦囊")	
			if(GetCurrentDay()~=LuaQueryStrTask("Ac17day")) then
				LuaSetTask("Ac17",0)
			end
			if(LuaQueryTask("Ac17")==0)then
				LuaSay("天道：今日目测大侠满脸红光，头顶祥云，必是招财进宝之日！")
				LuaGive("o_state005j",2,"Ac17")
				LuaGive("o_present020",1,"Ac17")
				LuaSetTask("Ac17",99)
				LuaSetTask("Ac17day",GetCurrentDay())
			else
				LuaSay("天道：大侠你今日已经领取过了")
			end
		end
	else
		LuaSay("活动期间内每日可在建康城城中心NPC天道领取一次奖励机不可失失不再来哦！每日仅有一次哦。奖品有：2颗暴击丸、1个恩赐锦囊")	
...
elseif(answer=="Ac18")then
	LuaSay("天道：刚说大侠头顶祥云，原来是要来购买[恩赐锦囊]，此锦囊却乃天下极品，能开出各种物品，小至经验符，大到顶级材料！无所不有！")
	LuaSay("天道：老朽年迈，只带了200个锦囊赠送给有缘人，你只需要花费20元宝就能获得一个")
	...
	elseif(answer=="Ac18_yes")then
		if(LuaQueryMoney()>=20)then
			...
			LuaSay("天道：小子，你身上的元宝木有20个呀，锦囊不能送给你")
		end



elseif(answer=="Ac19")then
	LuaSay("活动期间到外国杀怪可获得怪物的内丹，20个内丹可以兑换1个恩赐锦囊。恩赐锦囊在建康城城中心NPC天道兑换。锦囊可以随机开出各种高级材料，30级以上装备合成卷，以及各种高等级装备哦")
