...
if (answer=="ask") then	
	...
	if(LuaQueryTask("T1")<99 )  then
		...
		if(LuaQueryStr("MobieType") ~= "S20")then
			LuaSay("南宫若兰：您可以通过点击触摸屏进行行进，@3然后直接点我@0与我交谈。@!")
		else
		...
		if(LuaQueryTask("T1")==1)  then
			AddTopSayMenuItem("@3领取新手礼包@0","T1","1")
			havetask =1
		else
		...
		if(LuaQueryTask("T9")<99 and LuaQueryTask("T1")==99) then
			AddTopSayMenuItem("@3悬崖的秘密[lv13]@0","T9")
		end 
...
AddTopSayMenuItem("@3转生@0","ces")
AddTopSayMenuItem("1680元宝直升55级","JQHJY55_YES2")       
--return 1
elseif(answer=="JQHJY55_YES2")then
    local level= LuaQuery("level")
	if(level<55)then
        if(LuaQueryMoney()>=1680)then		
            LuaAddYB(1,-1680)
...
elseif(answer=="liaotian") then
	...
	UpdateMenu()
elseif(answer=="ces")then
	LuaSay("玩家到达100级可以转生,转生后回到1级,每次转生获得500点全属性,14级紫玉x2!")    
    LuaSay("每次转生后,名人堂需要重新挑战!")
    local lv=LuaQuery("level")
    if(lv==100)then        
        --zs1 1-9转状态
        local mzslv=LuaQueryTask("zs/mzslv")
        if(mzslv>0)then
            LuaDeleteBuff("zs1")
        else
            LuaSetTask("zs/mz",LuaQueryStr("name"))
        end
        local zsmz=LuaQueryStrTask("zs/mz")
        mzslv=mzslv+1
        LuaSetTask("zs/mzslv",mzslv)    
        LuaGiveBuff("zs1",mzslv)
        LuaSet("name",zsmz.."@3"..mzslv.."转") 
        LuaAddJx("combat_exp",LuaQuery("combat_exp")*-1,"zs")
        LuaSet("combat_exp",0)
        LuaSet("level",1)          
        LuaSay("转生成功!")
        LuaGive("o_present115",2)
        LuaAddJx("combat_exp",1,"zs")
    else
        LuaSay("当前等级"..lv.."无法进行转生,达到100级后再来!")
    end
	FlushMyInfo(0)
    FlushMyInfo(1)
    FlushMyInfo(2)
	...
	elseif(LuaQueryTask("T1")==99) then
 	LuaSay("南宫若兰：您已经领过新手礼包了，去追寻属于您的霸业吧。@2向右@0走到@3箭头@0处就可以去@4桃源径@0找@3冉闵@0了。")
		...
		elseif(answer=="T1jl")then
			...
			LuaSay("系统提示：恭喜您！获得了@4价值人民币500元@0的@4豪华新手大礼包@0一个！祝您游戏愉快！")
