...
if(answer=="ask") then
	...
	--AddTopSayMenuItem("@3[竞赛]查询排行(每日4:20更新)","sjbp")
	--AddTopSayMenuItem("@3[竞赛]排行奖励[2024-07-14 04:20结算]","sjbplj")
	AddTopSayMenuItem("@3上古卡牌","sgcardjh")
...
elseif(answer=="sjbp")then
		...
		if(r>10 or r<=0) then												 
			LuaSay("提示：加油吧少年,你的等级不在@3前10位@0")
		elseif(r==1)  then
			LuaSay("提示：恭喜你,你当前排名第一,请继续保持@3轩辕神剑[5孔]@0非你莫属(2024-7-14 4:20前有效)!")
			LuaNotice("等级榜@3第1位@0的大佬@2"..LuaQueryStr("name").."@0 正在健康城沾沾自喜,@3轩辕神剑[5孔]@0必是他囊中之物!。")
		elseif(r==2 or r==3)  then	
			LuaSay("提示：恭喜你,你当前排名@3第"..r.."位@0,马上就第一了,@3轩辕神剑[5孔]@0就在眼前!(2024-7-14 4:20前有效)")
			LuaNotice("等级榜@3第"..r.."位@0的玩家@2"..LuaQueryStr("name").."@0 正在健康城皱眉,@3轩辕神剑[5孔]@0似乎触手可及。")
		else
			LuaSay("提示：恭喜你,你当前排名@3第"..r.."位@0,离@3轩辕神剑[5孔]@0一定距离,努努力还是有可能的!(2024-7-14 4:20前有效)")
			LuaNotice("等级榜@3第"..r.."位@0的玩家@2"..LuaQueryStr("name").."@0 正在健康城苦苦思索,@3轩辕神剑[5孔]@0...?。")
		end
elseif(answer=="cardhecheng")then
    --LuaGive("o_cardjr",1)
    --LuaGive("o_cardxc",1)
    --LuaGive("o_cardzy",1)
    --LuaGive("o_cardtw",1)
    --LuaGive("o_present117",20)
    --LuaGive("o_present118",20)
    --LuaGive("o_present119",20)
    --LuaGive("o_present120",20)
    --LuaGive("o_present121",20)    
   AddMenuItem("消耗"..cardneed.."碎片合成随机整卡","")
   AddMenuItem("合成 白卡 碎片:"..LuaQueryTask("carexp/cardExp116"),"chcb")
   AddMenuItem("合成 @7绿卡@0 碎片:"..LuaQueryTask("carexp/cardExp117"),"chclv")
   AddMenuItem("合成 @8蓝卡@0 碎片:"..LuaQueryTask("carexp/cardExp118"),"chclan")
   AddMenuItem("合成 @2紫卡@0 碎片:"..LuaQueryTask("carexp/cardExp119"),"chczi")
   AddMenuItem("合成 @3金卡@0 碎片:"..LuaQueryTask("carexp/cardExp120"),"chcjin")
   AddMenuItem("合成 @6血卡@0 碎片:"..LuaQueryTask("carexp/cardExp121"),"chcxue")
elseif(answer=="chcb")then
   local bexp=LuaQueryTask("carexp/cardExp116")
   if(bexp>=cardneed)then
        local rd=LuaRandom(4)
        if(rd==0)then
            LuaGive("o_cardshen",1,"jhshp")
        elseif(rd==1)then
            LuaGive("o_cardzhen",1,"jhshp")
        elseif(rd==2)then
            LuaGive("o_cardkui",1,"jhshp")
        else
            LuaGive("o_cardjiao",1,"jhshp")
        end
        LuaSetTask("carexp/cardExp116",bexp-cardneed)
   else
        LuaSay("白卡碎片不足"..cardneed)
   end
elseif(answer=="chclv")then
   local bexp=LuaQueryTask("carexp/cardExp117")
   if(bexp>=cardneed)then
        local rd=LuaRandom(4)
        if(rd==0)then
            LuaGive("o_cardjr",1,"jhshp")
        elseif(rd==1)then
            LuaGive("o_cardxc",1,"jhshp")
        elseif(rd==2)then
            LuaGive("o_cardzy",1,"jhshp")
        else
            LuaGive("o_cardtw",1,"jhshp")
        end
        LuaSetTask("carexp/cardExp117",bexp-cardneed)
   else
        LuaSay("绿卡碎片不足"..cardneed)
   end
elseif(answer=="chclan")then
   local bexp=LuaQueryTask("carexp/cardExp118")
   if(bexp>=cardneed)then
        local rd=LuaRandom(4)
        if(rd==0)then
            LuaGive("o_cardjw",1,"jhshp")
        elseif(rd==1)then
            LuaGive("o_cardbf",1,"jhshp")
        elseif(rd==2)then
            LuaGive("o_cardlw",1,"jhshp")
        else
            LuaGive("o_cardyz",1,"jhshp")
        end
        LuaSetTask("carexp/cardExp118",bexp-cardneed)
   else
        LuaSay("蓝卡碎片不足"..cardneed)
   end
elseif(answer=="chczi")then
   local bexp=LuaQueryTask("carexp/cardExp119")
   if(bexp>=cardneed)then
        local rd=LuaRandom(4)
        if(rd==0)then
            LuaGive("o_cardfx",1,"jhshp")
        elseif(rd==1)then
            LuaGive("o_cardyzzi",1,"jhshp")
        elseif(rd==2)then
            LuaGive("o_cardcf",1,"jhshp")
        else
            LuaGive("o_cardliwen",1,"jhshp")
        end
        LuaSetTask("carexp/cardExp119",bexp-cardneed)
   else
        LuaSay("紫卡碎片不足"..cardneed)
   end
elseif(answer=="chcjin")then
   local bexp=LuaQueryTask("carexp/cardExp120")
   if(bexp>=cardneed)then
        local rd=LuaRandom(4)
        if(rd==0)then
            LuaGive("o_cardql",1,"jhshp")
        elseif(rd==1)then
            LuaGive("o_cardbz",1,"jhshp")
        elseif(rd==2)then
            LuaGive("o_cardjiuewi",1,"jhshp")
        else
            LuaGive("o_cardfh",1,"jhshp")
        end
        LuaSetTask("carexp/cardExp120",bexp-cardneed)
   else
        LuaSay("金卡碎片不足"..cardneed)
   end
elseif(answer=="chcxue")then
   local bexp=LuaQueryTask("carexp/cardExp121")
   if(bexp>=cardneed)then
        local rd=LuaRandom(4)
        if(rd==0)then
            LuaGive("o_cardpl",1,"jhshp")
        elseif(rd==1)then
            LuaGive("o_cardfz",1,"jhshp")
        elseif(rd==2)then
            LuaGive("o_cardtg",1,"jhshp")
        else
            LuaGive("o_cardh",1,"jhshp")
        end
        LuaSetTask("carexp/cardExp121",bexp-cardneed)
   else
        LuaSay("血卡碎片不足"..cardneed)
   end
elseif(answer=="sgcardjh")then
   AddMenuItem("消耗组合卡牌可激活卡牌组增加属性,后续消耗同样数量升级,每升级一次增加30%,最高5级","")
   AddMenuItem("卡牌合成,会员优惠20%","cardhecheng")
   local lv1=LuaQueryTask("card/shplv")
   local lv2=LuaQueryTask("card/mhplv")
   local lv3=LuaQueryTask("card/bailv")
   local lv4=LuaQueryTask("card/lvlv")
   local lv5=LuaQueryTask("card/lanlv")
   local lv6=LuaQueryTask("card/zilv")
   local lv7=LuaQueryTask("card/jinlv")  
   if(lv1>0 and LuaCheckBuff("592")==0)then    
        LuaGiveBuff("592",lv1)
   end
   if(lv2>0 and LuaCheckBuff("593")==0)then    
        LuaGiveBuff("593",lv2)
   end
   if(lv3>0 and LuaCheckBuff("594")==0)then    
        LuaGiveBuff("594",lv3)
   end
   if(lv4>0 and LuaCheckBuff("595")==0)then    
        LuaGiveBuff("595",lv4)
   end
   if(lv5>0 and LuaCheckBuff("596")==0)then    
        LuaGiveBuff("596",lv5)
   end
   if(lv6>0 and LuaCheckBuff("597")==0)then    
        LuaGiveBuff("597",lv6)
   end
   if(lv7>0 and LuaCheckBuff("598")==0)then    
        LuaGiveBuff("598",lv7)
   end
    FlushMyInfo(2)--刷新攻防
    FlushMyInfo(1)--刷新攻防
    FlushMyInfo(3)--刷新攻防
   AddMenuItem("@6小血卡 "..lv1,"jhshp") 
   AddMenuItem("@1大血卡 "..lv2,"jhmhp")    
   AddMenuItem("白卡 "..lv3,"jhbai") 
   AddMenuItem("@7绿卡 "..lv4,"jhlv") 
   AddMenuItem("@8蓝卡 "..lv5,"jhlan") 
   AddMenuItem("@2紫卡 "..lv6,"jhzi") 
   AddMenuItem("@3金卡 "..lv7,"jhjin") 
elseif(answer=="jhshp")then
    local pl=LuaItemCount("o_cardpl")
    local fz=LuaItemCount("o_cardfz")
    local tg=LuaItemCount("o_cardtg")
    local hk=LuaItemCount("o_cardh")
    local tlv=LuaQueryTask("card/shplv")
    AddMenuItem("需要@6[蒲牢卡 夫诸卡 天狗卡 吼卡]@0 激活加100000血蓝","")
    if(pl>0 and fz>0 and tg>0 and hk>0 and tlv<5)then
        LuaSetTask("card/shplv",tlv+1)
        DelItem("o_cardpl",1,"jhshp")
        DelItem("o_cardfz",1,"jhshp")
        DelItem("o_cardtg",1,"jhshp")
        DelItem("o_cardh",1,"jhshp")
        LuaSay("激活@6小血卡@0成功!")
        LuaDeleteBuff("592")
    else
        LuaSay("卡牌不足或已激活满5级")
    end
    AddMenuItem("返回","cardback")
elseif(answer=="jhmhp")then
    local pl=LuaItemCount("o_cardpl")
    local fz=LuaItemCount("o_cardfz")
    local tg=LuaItemCount("o_cardtg")
    local hk=LuaItemCount("o_cardh")
    local qq=LuaItemCount("o_cardqq")
    local tlv=LuaQueryTask("card/mhplv")
    AddMenuItem("需要@6[蒲牢卡 夫诸卡 天狗卡 吼卡 @1穷奇卡@0]@0 激活加500000血蓝","")
    if(pl>0 and fz>0 and tg>0 and hk>0 and qq>0 and tlv<5)then
        LuaSetTask("card/mhplv",tlv+1)
        DelItem("o_cardpl",1,"jhshp")
        DelItem("o_cardfz",1,"jhshp")
        DelItem("o_cardtg",1,"jhshp")
        DelItem("o_cardh",1,"jhshp")
        DelItem("o_cardqq",1,"jhshp")
        LuaSay("激活@1大血卡@0成功!")
        LuaDeleteBuff("593")
    else
        LuaSay("卡牌不足或已激活满5级")
    ...
    AddMenuItem("返回","cardback")
elseif(answer=="jhbai")then
    local pl=LuaItemCount("o_cardshen")
    local fz=LuaItemCount("o_cardzhen")
    local tg=LuaItemCount("o_cardkui")
    local hk=LuaItemCount("o_cardjiao")
    local tlv=LuaQueryTask("card/bailv")
    AddMenuItem("需要[蜃卡 狰卡 夔卡 鲛卡] 激活加5000攻 2500防 1250智力敏","")
    if(pl>0 and fz>0 and tg>0 and hk>0 and tlv<5)then
        LuaSetTask("card/bailv",tlv+1)
        DelItem("o_cardshen",1,"jhshp")
        DelItem("o_cardzhen",1,"jhshp")
        DelItem("o_cardkui",1,"jhshp")
        DelItem("o_cardjiao",1,"jhshp")
        LuaSay("激活白卡成功!")
        LuaDeleteBuff("594")
    else
        LuaSay("卡牌不足或已激活满5级")
        if(tlv==5)then
            LuaSay("多余的卡牌可以2:1转化为高级卡牌,转化后低级卡牌清零,获得高级卡牌取整数")
            AddMenuItem(LuaQueryTask("carexp/cardExp116").."转化为绿卡碎片","zhlvk")
        end
    end
    AddMenuItem("返回","cardback")
elseif(answer=="jhlv")then
    local pl=LuaItemCount("o_cardjr")
    local fz=LuaItemCount("o_cardxc")
    local tg=LuaItemCount("o_cardzy")
    local hk=LuaItemCount("o_cardtw")
    local tlv=LuaQueryTask("card/lvlv")
    AddMenuItem("需要@7[角瑞 獬豸 烛阴 涛杌]@0 激活加10000攻 5000防 2500智力敏","")
    if(pl>0 and fz>0 and tg>0 and hk>0 and tlv<5)then
        LuaSetTask("card/lvlv",tlv+1)
        DelItem("o_cardjr",1,"jhshp")
        DelItem("o_cardxc",1,"jhshp")
        DelItem("o_cardzy",1,"jhshp")
        DelItem("o_cardtw",1,"jhshp")
        LuaSay("激活@7绿卡@0成功!")
        LuaDeleteBuff("595")
    else
        LuaSay("卡牌不足或已激活满5级")
        if(tlv==5)then
            LuaSay("多余的卡牌可以2:1转化为高级卡牌,转化后低级卡牌清零,获得高级卡牌取整数")
            AddMenuItem(LuaQueryTask("carexp/cardExp117").."转化为蓝卡碎片","zhlank")
        end
    end
    AddMenuItem("返回","cardback")
elseif(answer=="jhlan")then
    local pl=LuaItemCount("o_cardjw")
    local fz=LuaItemCount("o_cardbf")
    local tg=LuaItemCount("o_cardlw")
    local hk=LuaItemCount("o_cardyz")
    local tlv=LuaQueryTask("card/lanlv")
    AddMenuItem("需要@8[精卫 毕方 陆吾 英招]@0 激活加20000攻 10000防 5000智力敏","")
    if(pl>0 and fz>0 and tg>0 and hk>0 and tlv<5)then
        LuaSetTask("card/lanlv",tlv+1)
        DelItem("o_cardjw",1,"jhshp")
        DelItem("o_cardbf",1,"jhshp")
        DelItem("o_cardlw",1,"jhshp")
        DelItem("o_cardyz",1,"jhshp")
        LuaSay("激活@8蓝卡@0成功!")
        LuaDeleteBuff("596")
    else
        LuaSay("卡牌不足或已激活满5级")
        ...
        if(tlv==5)then
            LuaSay("多余的卡牌可以2:1转化为高级卡牌,转化后低级卡牌清零,获得高级卡牌取整数")
            AddMenuItem(LuaQueryTask("carexp/cardExp118").."转化为紫卡碎片","zhzik")
        end
    end
    AddMenuItem("返回","cardback")
elseif(answer=="jhzi")then
    local pl=LuaItemCount("o_cardfx")
    local fz=LuaItemCount("o_cardyzzi")
    local tg=LuaItemCount("o_cardcf")
    local hk=LuaItemCount("o_cardliwen")
    local tlv=LuaQueryTask("card/zilv")
    AddMenuItem("需要@2[霸下 睚眦 嘲风 螭吻]@0 激活加50000攻 25000防 12500智力敏","")
    if(pl>0 and fz>0 and tg>0 and hk>0 and tlv<5)then
        LuaSetTask("card/zilv",tlv+1)
        DelItem("o_cardfx",1,"jhshp")
        DelItem("o_cardyzzi",1,"jhshp")
        DelItem("o_cardcf",1,"jhshp")
        DelItem("o_cardliwen",1,"jhshp")
        LuaSay("激活@2紫卡@0成功!")
        LuaDeleteBuff("597")
    else
        LuaSay("卡牌不足或已激活满5级")
        if(tlv==5)then
            LuaSay("多余的卡牌可以2:1转化为高级卡牌,转化后低级卡牌清零,获得高级卡牌取整数")
            AddMenuItem(LuaQueryTask("carexp/cardExp119").."转化为金卡碎片","zhjink")
        end
    end
    AddMenuItem("返回","cardback")
elseif(answer=="jhjin")then
    local pl=LuaItemCount("o_cardql")
    local fz=LuaItemCount("o_cardbz")
    local tg=LuaItemCount("o_cardjiuewi")
    local hk=LuaItemCount("o_cardfh")
    local tlv=LuaQueryTask("card/jinlv")
    AddMenuItem("需要@3[麒麟 白泽 九尾 凤凰]@0 激活加100000攻 50000防 25000智力敏","")
    if(pl>0 and fz>0 and tg>0 and hk>0 and tlv<5)then
        LuaSetTask("card/jinlv",tlv+1)
        DelItem("o_cardql",1,"jhshp")
        DelItem("o_cardbz",1,"jhshp")
        DelItem("o_cardjiuewi",1,"jhshp")
        DelItem("o_cardfh",1,"jhshp")
        LuaSay("激活@3金卡@0成功!")
        LuaDeleteBuff("598")
    else
        LuaSay("卡牌不足或已激活满5级")
    ...
    AddMenuItem("返回","cardback")    
elseif(answer=="cardback")then
    do_talk("sgcardjh")
elseif(answer=="zhlvk")then
    local carexp="carexp/cardExp"
    local card1=LuaQueryTask(carexp.."116")/2
    if(card1>1)then
        LuaSetTask(carexp.."116",0)
        local card2=LuaQueryTask(carexp.."117")+card1
        LuaSetTask(carexp.."117",card2)
        LuaSay("失去所有白卡碎片,获得"..card1.."绿卡碎片")
    else
        LuaSay("白卡碎片不足10个无法转化!")
    end
elseif(answer=="zhlank")then
    local carexp="carexp/cardExp"
    local card1=LuaQueryTask(carexp.."117")/2
    if(card1>1)then
        LuaSetTask(carexp.."117",0)
        local card2=LuaQueryTask(carexp.."118")+card1
        LuaSetTask(carexp.."118",card2)
        LuaSay("失去所有绿卡碎片,获得"..card1.."蓝卡碎片")
    else
        LuaSay("绿卡碎片不足10个无法转化!")
    end
elseif(answer=="zhzik")then
    local carexp="carexp/cardExp"
    local card1=LuaQueryTask(carexp.."118")/2
    if(card1>1)then
        LuaSetTask(carexp.."118",0)
        local card2=LuaQueryTask(carexp.."119")+card1
        LuaSetTask(carexp.."119",card2)
        LuaSay("失去所有蓝卡碎片,获得"..card1.."紫卡碎片")
    else
        LuaSay("白卡碎片不足10个无法转化!")
    end
elseif(answer=="zhjink")then
    local carexp="carexp/cardExp"
    local card1=LuaQueryTask(carexp.."119")/2
    if(card1>1)then
        LuaSetTask(carexp.."119",0)
        local card2=LuaQueryTask(carexp.."120")+card1
        LuaSetTask(carexp.."120",card2)
        LuaSay("失去所有紫卡碎片,获得"..card1.."金卡碎片")
    else
        LuaSay("紫卡碎片不足10个无法转化!")
    end
elseif(answer=="sjbplj")then
		--领奖
		if(GetCurrentDay()~="2024-7-14" or GetCurrentHour()<9)then
			LuaSay("不在领取时间范围内 2024-7-14 9点后可以领取!")
		else
			if(LuaQueryTask("sjbplj")==99)then
				LuaSay("你已领取过,该活动只能领取一次,期待下次活动吧!")
			else
				local r=RangeQuery("experice")	                                                         
				if(r>10 or r<=0) then												 
					LuaSay("提示：无法领取,你的等级排行不在@3前10位@0")
				elseif(r==1)  then
					LuaSay("提示：恭喜你,你当前排名第一,得到@3轩辕神剑[5孔]x1@0及其它材料物品!")
					LuaGive("e_xysj",1,5,"sjbplj")
					LuaGive("o_present032",1,"sjbplj")--6级单一材料包
					LuaGive("o_ybsc002y",20,"sjbplj")--强化石
					LuaGive("o_ybsc001y",20,"sjbplj")--拆解符
					LuaGive("o_lv70taozhuang",3,"sjbplj")--70级套装合成书
					LuaNotice("恭喜等级榜@3第1位@0的大佬@2"..LuaQueryStr("name").."@0 在健康城[@2等级竞赛@0]活动中领取了@3轩辕神剑[5孔]x1@0及其它材料物品!。")
				elseif(r==2 or r==3)  then	
					LuaSay("提示：由于你在等级榜排@3第"..r.."位@0，可获得@3120W@0的铜币奖励及其它材料物品。")
					LuaGive("coin",1200000,"sjbplj")
					LuaGive("o_present030",1,"sjbplj")
					LuaGive("o_ybsc002y",10,"sjbplj")
					LuaGive("o_ybsc001y",10,"sjbplj")
					LuaGive("o_lv70taozhuang",2,"sjbplj")
					LuaNotice("等级榜@3第"..r.."位@0的玩家@2"..LuaQueryStr("name").."@0 在健康城[@2等级竞赛@0]活动中领取了@3120W@0铜币及其它材料物品。")
				else
					LuaSay("提示：由于你在等级榜排@3第"..r.."位@0，可获得@365W@0的铜币奖励及其它材料物品。")
					LuaGive("coin",650000,"Tpaihang")
					LuaGive("o_present028",5,"sjbplj")
					LuaGive("o_ybsc002y",5,"sjbplj")
					LuaGive("o_ybsc001y",5,"sjbplj")
					LuaGive("o_lv70taozhuang",1,"sjbplj")
				end
				LuaSetTask("sjbplj",99)
			end
		end
