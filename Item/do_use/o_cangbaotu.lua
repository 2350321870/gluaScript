...
local cy=LuaQueryTemp("posy")
	if(cx>px-16 and cx<px+16 and cy>py-16 and cy<py+16) then
		...
		r=LuaRandom(30)
		if(r<4)then
			LuaGive("o_material_"..(13+9*LuaRandom(6)).."",1,"Tcangbaotu")
		elseif(r==4 or r==5) then
			LuaGive("o_material_"..(14+9*LuaRandom(6)).."",1,"Tcangbaotu")
			LuaNotice("@2"..LuaQueryStr("name").."@0在寻宝时寻到@24级材料@0一个")
		elseif(r==6) then
			LuaGive("o_material_"..(15+9*LuaRandom(6)).."",1,"Tcangbaotu")
			LuaNotice("@2"..LuaQueryStr("name").."@0在寻宝时寻到@25级材料@0一个")
		--elseif(r<=16)then
			--local jingyan=(32818+level^2*20)
			--LuaAddJx("combat_exp",jingyan,"Tcangbaotu")
		else
			if(level<30)then
				LuaGive("coin",(LuaRandom(3)+3)*30*level,"Tcangbaotu")
			elseif(level<40)then
				LuaGive("coin",(LuaRandom(4)+3)*20*level,"Tcangbaotu")
			else
				LuaGive("coin",(LuaRandom(5)+3)*10*level,"Tcangbaotu")
			end
		end
		if(LuaQueryTask("Thx2")==1) then
			if(LuaQueryTask("Thx2A")==0) then
				LuaSay("提示：你已经完成环形任务第一环，快去回复拓跋圭吧。@!")
				LuaSetTask("Thx2A",99)
			end
		end
		if(LuaRandom(20)==0)then
			LuaGive("o_mission018",1,"Tcangbaotu")
		end
		LuaDelTask("Tbaotu")
		LuaOtherSendMenu()
		return 1
