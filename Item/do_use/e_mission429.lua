...
task[319]={319,"魔王殿","lu_mwd","Tmap_41","","color",1}
	local e=LuaQueryTask("task5type5col")
	if(LuaQueryTask(task[e][4])==1 or LuaQueryTask(task[e][4])==2)then
		if(MapName()==task[e][3])then
			LuaSay("提示：已经通知到当地的群众了，你可以回去交差了")
			LuaSetTask(task[e][4],2)
			LuaOtherSendMenu()
			return 1
		else
			LuaSay("提示：你需要去的地方是"..task[e][2].."")
			LuaOtherSendMenu()
		end
	else
		return 0
