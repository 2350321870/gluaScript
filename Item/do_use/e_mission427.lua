...
task[319]={319,"魔王殿","lu_mwd","Tmap_41","","color",1}
	local c=LuaQueryTask("task3type5col")	
	if(LuaQueryTask(task[c][4])==1 or LuaQueryTask(task[c][4])==2)then
		if(MapName()==task[c][3])then
			LuaSay("提示：已经通知到当地的群众了，你可以回去交差了")
			LuaSetTask(task[c][4],2)
			LuaOtherSendMenu()
			return 1
		else
			LuaSay("提示：你需要去的地方是"..task[c][2].."")
			LuaOtherSendMenu()
		end
	else
		return 0
