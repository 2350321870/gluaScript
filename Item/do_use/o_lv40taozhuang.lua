function do_use()
	...
	if(LuaFreeBag()>=2)then
		AddMenuItem("合成说明","nc_combine o_lv40taozhuang shuoming","40级强者套装合成书")
		AddMenuItem("合成指南","nc_combine o_lv40taozhuang zhinan","40级强者套装合成书")
  		local num=1
  		while(num<=12)
  		do
			if(dy>=hc[num][1] and ly>=hc[num][4]) then
			AddMenuItem(""..hc[num][9].."[@7可合成@0]","nc_combine o_lv40taozhuang flag["..num.."]","40级强者套装合成书")
			else
			AddMenuItem(""..hc[num][9].."[@2需要"..(hc[num][1]).."个"..(hc[num][3]).."，需要"..hc[num][4].."个"..(hc[num][6]).."@0]","nc_combine o_lv40taozhuang flag["..num.."]","40级强者套装合成书")
			end
...
LuaSay("可以合出40级强者套装，装备的品质较大概率为绿色，一定概率为蓝色，较小概率为黄金！！合成时请一定留出足够的包裹空间，不然会造成丢失哦！")
