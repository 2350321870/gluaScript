...
if(EquipInfo(10)~="")then
	 if(GetCurrentDay()~=LuaQueryStrTask("food03j_day"))then
	   LuaDelTask("food03_num")
         end
	
	local num = LuaQueryTask("food03j_num")    --今日适使用次数
	local pet_lv=LuaPetQuery("lv")
	local pet_maxlv=LuaQuery("level")
	local exp_get=LuaPetQuery("exp_get")
 	local a=floor((pet_lv^2*2+pet_lv*250)*(1+exp_get/100))--经验调整处
	local addexp=3*a
	...
	if(pet_lv>=30)then
		if(num<10)then
			if((pet_lv<pet_maxlv) and (LuaItemCount("o_food03j")>=1))  then
				LuaSay("今日您第"..(num+1).."次使用宠物食品,将会得到经验@3["..addexp1.."]@0,")
				LuaPetExpAdd(addexp1)
			
				LuaSay("恭喜您今天第"..(num+1).."次获得宠物经验，下一次使用可获得经验为"..addexp2.."")
				LuaAddTask("food03j_num",1)
				LuaSetTask("food03j_day",GetCurrentDay())
				LuaOtherSendMenu()
				return 1
			else
